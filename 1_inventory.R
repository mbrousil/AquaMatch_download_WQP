...
p1_targets_list <- list(
  ...
  tar_file(
    ...
    command = export_single_file(target = p1_wqp_params,
  ...
  # version is needed.
  tar_file(
    name = p1_wqp_params_link_stable,
    command = {
      ...
      stable_drive_links <- tribble(
        ...
        "p1_wqp_params", "1_inventory/out/p1_wqp_params.rds", "https://drive.google.com/file/d/1glySy4t8K5LLwRmgG_-PGUtTltMbkT9r/view?usp=drive_link"
      )
  ...
  tar_target(
    ...
    command = crosswalk_characteristics(p1_wqp_params)
  ),
  ...
  tar_file(
    ...
    command = export_single_file(target = p1_char_names_crosswalk,
                                 folder_pattern = "1_inventory/out/")
  ),
  ...
  tar_file(
    name = p1_char_names_crosswalk_link_stable,
    command = {
      # Where the Drive link csv is going
      ...
      stable_drive_links <- tribble(
        ~dataset, ~local_path, ~drive_link,
        "p1_char_names_crosswalk", "1_inventory/out/p1_char_names_crosswalk.rds", "https://drive.google.com/file/d/1aetAWgtIuIVB9YOOCzlTuESM1o40hT1B/view?usp=drive_link"
      )
  ...
  tar_target(
    name = p1_char_names,
    command = filter_characteristics(p1_char_names_crosswalk, p0_param_groups_select),
  ...
  tar_file(
    name = p1_similar_char_names_txt,
    command = find_similar_characteristics(p1_char_names,
  ...
  tar_file(
    ...
    command = {
      ...
      stable_drive_links <- tribble(
        ~dataset, ~local_path, ~drive_link,
        "p1_global_grid", "1_inventory/out/p1_global_grid.rds", "https://drive.google.com/file/d/1zKgAzXpWwMiVvJT2IMHW6kou5ijDsqgO/view?usp=drive_link"
      )
  ...
  tar_target(
    ...
    command = subset_inventory(p1_wqp_inventory, p1_AOI_sf)
  ),
  ...
  tar_file(
    ...
    command = export_single_file(target = p1_wqp_inventory_aoi,
                                 folder_pattern = "1_inventory/out/")
  ),
  # An alternative to the link generated in the step above. This option provides
  ...
  # version is needed.
  tar_file(
    name = p1_wqp_inventory_aoi_link_stable,
    command = {
      # Where the Drive link csv is going
      ...
      stable_drive_links <- tribble(
        ~dataset, ~local_path, ~drive_link,
        "p1_wqp_inventory_aoi", "1_inventory/out/p1_wqp_inventory_aoi.rds", "https://drive.google.com/file/d/1hZnH6ywFlQh_TCqXaqE70wHpLdoKTH3m/view?usp=drive_link"
      )
  ...
  tar_file(
    name = p1_wqp_inventory_summary_csv,
    command = summarize_wqp_inventory(p1_wqp_inventory_aoi,
